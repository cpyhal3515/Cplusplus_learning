...
public:
	...
	{
		std::cout << "执行 Sales_data(const std::string& s, unsigned n, double p)" << std::endl;
	};
	// 委托构造函数：将实际的构造任务交给构造函数完成
	Sales_data() : Sales_data(" ", 0, 0) 
	{ 
		std::cout << "执行 Sales_data()" << std::endl;
	};
	Sales_data(const std::string& s) : Sales_data(s, 0, 0) \
	{ 
		std::cout << "执行 Sales_data(const std::string& s)" << std::endl;
	};
	Sales_data(std::istream& is) :Sales_data() 
	{ 
		read(is, *this); 
		std::cout << "执行 Sales_data(std::istream& is)" << std::endl;
	};
